...
const ManageResumes = () => {
  ...
  const handleSave = () => {
    const updatedApps = applications.map((app) =>
      app._id === editingApp._id ? { ...app, note, status } : app
    );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      // Remove the application from state after deletion
      ...
      <div className="main-content p-6">
        <div className="bg-white rounded-lg shadow-sm divide-y divide-gray-200">
          {visibleApplications.map((app) => (
            <div key={app._id} className="flex items-start gap-4 p-4 hover:bg-gray-50 transition">
              <img src={app.image_url} alt={app.teacher_name} className="w-28 h-36 object-cover rounded-md ml-4" />
              <div className="flex-grow">
                ...
                <div className="text-sm text-gray-500 mt-1 space-y-1">
                  ...
                  <p><strong>Hourly Rate:</strong> $30</p>
                  <p><strong>Location:</strong> 📍 Cupertino, CA, USA</p>
                </div>
                ...
                <p className="text-xs mt-1 text-gray-500"><strong>Status:</strong> {app.status || "New"}</p>
                {app.note && <p className="text-xs mt-1 text-gray-500"><strong>Note:</strong> {app.note}</p>}
              </div>
        ...
        <div className="flex justify-center mt-10 space-x-1">
          <button onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))} disabled={currentPage === 1}
            className={`px-3 py-1 border rounded w-8 h-8 flex items-center justify-center ${
              currentPage === 1 ? "bg-white text-gray-300 border-gray-300 cursor-not-allowed" : "hover:bg-gray-100 border-gray-300 text-gray-600"
            }`}>
            ←
          </button>
          {Array.from({ length: totalPages }).map((_, index) => (
            <button key={index + 1} onClick={() => setCurrentPage(index + 1)}
              className={`px-3 py-1 border rounded w-8 h-8 flex items-center justify-center font-medium ${
                currentPage === index + 1 ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-700 border-gray-300 hover:bg-gray-100"
              }`}>
              {index + 1}
            </button>
          ))}
          <button onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages}
            className={`px-3 py-1 border rounded w-8 h-8 flex items-center justify-center ${
              currentPage === totalPages ? "bg-white text-gray-300 border-gray-300 cursor-not-allowed" : "hover:bg-gray-100 border-gray-300 text-gray-600"
            }`}>
            →
          </button>
        </div>
            ...
            <div className="bg-white p-6 rounded-lg w-96 space-y-4 shadow-lg">
              ...
              <div className="flex flex-wrap items-start gap-4">
                <div className="w-40">
                  ...
                  <select value={status} onChange={(e) => setStatus(e.target.value)} className="w-full border px-3 py-2 rounded text-sm">
                ...
                <div className="flex-grow">
                  ...
                  <input type="text" value={note} onChange={(e) => setNote(e.target.value)} className="w-full border px-3 py-2 rounded text-sm" placeholder="Add note (optional)" />
                </div>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={() => setEditingApp(null)} className="px-4 py-2 text-sm rounded border">Cancel</button>
